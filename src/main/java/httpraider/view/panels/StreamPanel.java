...
public class StreamPanel extends JPanel {
    ...
    public StreamPanel(){
        ...
        proxyRequests.add(new HTTPEditorPanel<>("Proxy 1",HTTPRaiderExtension.API.userInterface().createHttpRequestEditor(EditorOptions.READ_ONLY)));
        proxyRequests.add(new HTTPEditorPanel<>("Proxy 2",HTTPRaiderExtension.API.userInterface().createHttpRequestEditor(EditorOptions.READ_ONLY)));
    ...
    private JSplitPane getNestedRequestPane(){
        ...
        if(proxyRequests.isEmpty()) return root;
    ...
    public ConnectionBar getConnectionBar(){ return connectionBar; }

    public void bindEditorsToModel(Stream m){
        clientRequest.addByteListener(m::setClientRequest);
        requestQueue.addByteListener(m::setRequestQueue);
        responseQueue.addByteListener(m::setResponseQueue);
    }

    public ConnectionSettings snapshotConnectionSettings(){
        return new ConnectionSettings(connectionBar.getHost(),connectionBar.getPort(),connectionBar.isHttps(),connectionBar.isReset());
    }

    public void updateStatus(boolean connected){ connectionBar.setConnected(connected); }
}
